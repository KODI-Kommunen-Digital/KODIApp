...
class _DiscoveryScreenState extends State<DiscoveryScreenDetail> {
  ...
  Future<void> loadLocationList() async {
    await context.read<DiscoveryCubit>().onLoad(id: widget.arguments['id']);
  }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      onPopInvokedWithResult: (bool didPop, dynamic result) {
        context.read<DiscoveryCubit>().onLoad();
      },
      child: Scaffold(
        appBar: AppBar(
          centerTitle: true,
...
class _DiscoveryLoadedState extends State<DiscoveryLoaded> {
  ...
  Future<void> navigateToLink(CitizenServiceModel service) async {
    if (service.arguments == 150) {
      launchUrl(
          Uri.parse(
              "https://freiraum-fichtelgebirge.de/freiraum-fuer-macher/macher-stories/"),
          mode: LaunchMode.inAppWebView);
    } else if (service.arguments == 151) {
      launchUrl(Uri.parse("https://freiraum-fichtelgebirge.de/fichtelpodcast/"),
          mode: LaunchMode.inAppWebView);
    }
  }
